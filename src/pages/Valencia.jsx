...
const Valencia = () => {
 ...
 function busqueda() {
  ...
  for (var i = 0; i < 4; i++) {
   ...
   if (i === randomPos) {
    ...
    valElemento = eleccion.grupo[Math.floor(Math.random() * eleccion.grupo.length)]
   }

   elements.push(eleccion.valencia)
  }
  setActual({
 ...
 return (
  <div className="relative">
   ...
   <div className={`${(acierto.click === 1 && ' blur-lg')} flex flex-col items-center bg-slate-800 h-screen`}>
    <div className="my-5">
    ...
    <div className="py-2 mt-16 px-3 flex flex-col gap-11 items-center">
     <div className="bg-slate-200 rounded-lg w-fit h-fit p-2 text-slate-900 flex items-center justify-center">
      <h3 className="text-7xl font-semibold">{actual.elemento}</h3>
     </div>
